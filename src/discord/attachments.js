/**
 * attachments.js — extract file references from text, validate against an allowlist,
 * and return discord.js AttachmentBuilder objects + cleaned text.
 *
 * Exported: extractAttachments(text, opts = {})
 *   returns: { cleanedText, files: AttachmentBuilder[], dropped: [{ path, reason }] }
 *
 * File reference formats recognised:
 ...
 * AttachmentBuilder objects plus cleaned text.
 *
 ...
 * @param {number} [opts.maxBytes] — max file size in bytes (default 8MB)
...
export function extractAttachments(text, opts = {}) {
  ...
  // ── Pass 3: build AttachmentBuilders and collect removal ranges ───────────
  ...
  for (const m of text.matchAll(/!\[.*?\]\((file:\/\/|)(\/[^)]+)\)/gs)) {
    ...
    if (resolved && acceptedPaths.has(resolved) && !isDropped(resolved)) {
      ...
      files.push(new AttachmentBuilder(resolved, { name: path.basename(resolved) }));
      fileCount++;
    }
  }
